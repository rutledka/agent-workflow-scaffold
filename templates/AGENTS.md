...
8. **Do not merge your own PR.** Leave it open for review unless the user explicitly asks you to merge. **Merges belong to the evaluator persona** (the one marked `role: evaluator` in `orchestration/graph.yaml`, usually QA), and only through `scripts/qa-merge.sh` — which verifies green checks on the PR's current head SHA, a valid verdict artifact, the no-self-merge rule, and the design-PR hold before merging. See the Autonomy table below.
...
- **Merge authority is the evaluator persona's, via `scripts/qa-merge.sh` only** — a gate is a command with an exit code, not a comment convention. No agent merges a design-hold PR (per `orchestration/graph.yaml` `policy.design_hold`); those wait for a human eye.
...
Who decides what. This binds **every agent** — primary sessions, sub-agents, scheduled runs. When in doubt about whether an action is yours to take, it isn't: surface it.

| Action class | Decides | Notes |
|---|---|---|
| Product roadmap, milestone scope + exit criteria, epic creation/priority | **Human** | `pm/roadmap.md` is a deliberate human artifact — agents plan within it, never rewrite it |
| Ticket decomposition, dependency edges, sizing, next-task selection | **Agent** (PM/orchestrator personas) | Within human-set milestone scope; selection is scored via `orchestration/graph.yaml` weights |
| Status sync, reconciliation, dispatch logs, label hygiene, worktree pruning | **Agent, unattended** | The invariant checks in `scripts/graph-sync.sh` — automate, don't remember |
| Write code, open PR | **Agent** | Worktree → PR, always |
| **Merge to the default branch** | **Evaluator persona**, via `scripts/qa-merge.sh` | Never the author; never a red or stale head |
| Design-PR merges; prod deploys; destructive schema changes; spend beyond declared budgets; external comms | **Human** | Design work gets a human eye before merge — no exceptions |
...
orchestration/  — the work graph as data: graph.yaml (policy — personas, resources,
                  gates, budgets), state.json (derived — regenerated by graph-sync,
                  never hand-edited), schemas/ (dispatch-result + qa-verdict contracts)
pm/             — product backlog, roadmap, management notes, codebase registry, goals
scripts/        — loop guard rails: preflight, graph-sync, with-resource-lock,
                  policy-lint, qa-merge (see scripts/README.md)
...
## Key Documents
- `orchestration/graph.yaml` — the policy graph: persona↔work ownership, file domains, resources, gates, budgets. **Ownership lives here** (and flows to PM-tool labels and generated docs); **status lives in the PM tool** (and flows the other way). Neither is bidirectional sync.
