...
- API contract document (typically `docs/api-specification.md` — confirm path against `CLAUDE.md` Project-specific rules section).
...
- **Validate at the edge, trust the inside.** Inputs from clients (request bodies, query params, path params) are validated using the project's chosen schema library before they touch a database, cache, or external service. See `CLAUDE.md` Project-specific rules for the exact rule.
...
- **Migrations are forward-only by default.** Destructive schema changes (drop column, rename column, change type) split into two PRs: first PR adds the new shape; a later PR removes the old. See `CLAUDE.md` Project-specific rules.
...
## Key References
- `CLAUDE.md` — git workflow, hard rules, project-specific rules (Zod / no-console / migration / etc.).
