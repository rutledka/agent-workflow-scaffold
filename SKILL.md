...
1. **`orchestration/graph.yaml`** — render `templates/orchestration/graph.yaml` from the confirmed 3f plan: one persona block per confirmed persona (file, `file_domains`, starter caps), `role: evaluator` on the evaluator, `dispatch: interactive-only` on design personas, the Q16 resources with their recorded reasons, the confirmed gates, `merge_gate_mode` from the Step 1 probe, and the signed-off budget numbers. **When the project's branch naming doesn't use full persona slugs as prefixes** (check `git branch -r` — short forms like `backend/`, `qa/`, `design/` are common), add `branch_prefix` to each persona block; `qa-merge.sh`'s no-self-merge check and `graph-sync.sh`'s PR attribution resolve authorship through it and silently mis-resolve without it. Delete the template's commented example blocks from the rendered output.
